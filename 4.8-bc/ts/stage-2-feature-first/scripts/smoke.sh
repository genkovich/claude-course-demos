#!/usr/bin/env bash
# Smoke test для stage-2-feature-first: hit 6 endpoints, expect all 2xx.
...
ok "register → 201, user_id=$USER_ID"
...
ORDER=$(curl -sS -o /tmp/smoke-order.json -w "%{http_code}" \
    ...
    "${BASE_URL}/orders")
[ "$ORDER" = "201" ] || fail "order expected 201 got $ORDER ($(cat /tmp/smoke-order.json))"
ok "order → 201"
